...
export default function DetailedScorecardPage() {
  ...
  if (!attempt) {
    return (
      <div className="max-w-4xl mx-auto px-4 py-12 text-center space-y-4">
        <h2 className="text-base font-bold text-white">Scorecard Loading...</h2>
        <p className="text-xs text-slate-400">Fetching attempt details.</p>
        <Link href="/results" className="px-4 py-2 bg-blue-600 text-white text-xs font-semibold rounded-xl inline-block">
          Return to Results History
        </Link>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-800 pb-6">
        <div>
          <Link href="/results" className="text-xs text-blue-400 font-semibold flex items-center space-x-1 mb-2 hover:underline">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 space-y-6 shadow-xl">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-800 pb-4">
          <div>
            ...
            <p className="text-xs text-slate-400">Review your locked choice vs correct answer for all 100 questions</p>
          </div>
              ...
              <div key={q.id} className="bg-slate-950 border border-slate-800 rounded-2xl p-5 space-y-4">
                  ...
                  {(['A', 'B', 'C', 'D', 'E'] as const).map(opt => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {isUserChoice && !isRightOption && <span className="text-[10px] bg-red-500 text-white font-bold px-2 py-0.5 rounded">Your Locked Choice</span>}
                      </div>
                    );
                  })}
                </div>
                ...
                {q.explanation && (
                  <div className="bg-slate-900/80 p-3.5 rounded-xl border border-slate-800 text-xs text-slate-300 space-y-1">
                    <span className="font-bold text-blue-400 block">Solution Step-by-Step:</span>
